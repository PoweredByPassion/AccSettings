...
class AccHandlerTest {
    ...
    fun buildInstallCommand_runsFromScriptDirectory() {
        val script = File("/data/user/0/app.owlow.accsetting/cache/install-tarball.sh")

        assertEquals(
            "cd '/data/user/0/app.owlow.accsetting/cache' && sh './install-tarball.sh' acc",
    ...
    fun buildCacheCleanupCommand_removesAllCacheEntries() {
        assertEquals(
            "find '/data/user/0/app.owlow.accsetting/cache' -mindepth 1 -maxdepth 1 -exec rm -rf {} +",
            AccHandler.buildCacheCleanupCommand(File("/data/user/0/app.owlow.accsetting/cache"))
        )
    }
